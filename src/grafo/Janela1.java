/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package grafo;

import javax.swing.JOptionPane;

/**
 *
 * @author hugo
 */
public class Janela1 extends javax.swing.JFrame {

    Grafo g;

    /**
     * Creates new form Janela1
     */
    public Janela1() {
        initComponents();
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jLabel2 = new javax.swing.JLabel();
        jLabel3 = new javax.swing.JLabel();
        cmbOrigem = new javax.swing.JComboBox();
        cmbDestino = new javax.swing.JComboBox();
        jLabel4 = new javax.swing.JLabel();
        jScrollPane1 = new javax.swing.JScrollPane();
        txtarGrafo = new javax.swing.JTextArea();
        jLabel5 = new javax.swing.JLabel();
        txtPeso = new javax.swing.JTextField();
        btnInserirAresta = new javax.swing.JButton();
        jLabel6 = new javax.swing.JLabel();
        jScrollPane2 = new javax.swing.JScrollPane();
        txtarDijkstra = new javax.swing.JTextArea();
        jLabel7 = new javax.swing.JLabel();
        txtVerticeInicial = new javax.swing.JTextField();
        txtMenorCaminho = new javax.swing.JButton();
        jLabel8 = new javax.swing.JLabel();
        txtQtdeVertices = new javax.swing.JTextField();
        btnCriarVertices = new javax.swing.JButton();
        jButton1 = new javax.swing.JButton();
        jLabel9 = new javax.swing.JLabel();
        jLabel10 = new javax.swing.JLabel();
        btnCalcularSomaPeso = new javax.swing.JButton();
        numDeVertices1 = new javax.swing.JTextField();
        respCalcSomPes = new javax.swing.JTextField();
        textBox = new javax.swing.JTextField();
        pesquisaNoGrafo = new javax.swing.JButton();
        entrarVertice = new javax.swing.JTextField();
        mostraAdjacentes = new javax.swing.JButton();
        jLabel11 = new javax.swing.JLabel();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);

        jLabel2.setText("Origem:");

        jLabel3.setText("Destino:");

        jLabel4.setText("Grafo:");

        txtarGrafo.setColumns(20);
        txtarGrafo.setRows(5);
        jScrollPane1.setViewportView(txtarGrafo);

        jLabel5.setText("Peso:");

        btnInserirAresta.setText("Inserir Aresta");
        btnInserirAresta.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnInserirArestaActionPerformed(evt);
            }
        });

        jLabel6.setText("Dijkstra:");

        txtarDijkstra.setColumns(20);
        txtarDijkstra.setRows(5);
        jScrollPane2.setViewportView(txtarDijkstra);

        jLabel7.setText("Vértice Inicial:");

        txtMenorCaminho.setText("Menor Caminho Dijkstra");
        txtMenorCaminho.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                txtMenorCaminhoActionPerformed(evt);
            }
        });

        jLabel8.setText("Quantidade de vértices que deseja criar:");

        btnCriarVertices.setText("Criar");
        btnCriarVertices.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnCriarVerticesActionPerformed(evt);
            }
        });

        jButton1.setText("TESTE!!!!");
        jButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton1ActionPerformed(evt);
            }
        });

        jLabel9.setText("Entre com o numero do vertice:");

        jLabel10.setText("Resposta :");

        btnCalcularSomaPeso.setText("Calcular Soma Peso");
        btnCalcularSomaPeso.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnCalcularSomaPesoActionPerformed(evt);
            }
        });

        numDeVertices1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                numDeVertices1ActionPerformed(evt);
            }
        });

        respCalcSomPes.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                respCalcSomPesActionPerformed(evt);
            }
        });

        textBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                textBoxActionPerformed(evt);
            }
        });

        pesquisaNoGrafo.setText("Pesquisa no grafo");
        pesquisaNoGrafo.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                pesquisaNoGrafoActionPerformed(evt);
            }
        });

        entrarVertice.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                entrarVerticeActionPerformed(evt);
            }
        });

        mostraAdjacentes.setText("Mostra Adjacentes");
        mostraAdjacentes.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mostraAdjacentesActionPerformed(evt);
            }
        });

        jLabel11.setText("Entrar Vertice:");

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jLabel8)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(txtQtdeVertices, javax.swing.GroupLayout.PREFERRED_SIZE, 50, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                        .addComponent(btnCriarVertices)
                        .addGap(49, 49, 49)
                        .addComponent(jButton1))
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(layout.createSequentialGroup()
                            .addComponent(jLabel2)
                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                            .addComponent(cmbOrigem, javax.swing.GroupLayout.PREFERRED_SIZE, 50, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                            .addComponent(jLabel3)
                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                            .addComponent(cmbDestino, javax.swing.GroupLayout.PREFERRED_SIZE, 50, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGap(18, 18, 18)
                            .addComponent(jLabel5)
                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                            .addComponent(txtPeso, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGap(18, 18, 18)
                            .addComponent(btnInserirAresta, javax.swing.GroupLayout.PREFERRED_SIZE, 120, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(jLabel4)
                                .addGap(24, 24, 24)
                                .addComponent(jScrollPane1))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(jLabel6)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(jScrollPane2, javax.swing.GroupLayout.PREFERRED_SIZE, 405, javax.swing.GroupLayout.PREFERRED_SIZE)))
                        .addGroup(layout.createSequentialGroup()
                            .addComponent(jLabel7)
                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                            .addComponent(txtVerticeInicial, javax.swing.GroupLayout.PREFERRED_SIZE, 50, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                            .addComponent(txtMenorCaminho))))
                .addGap(42, 42, 42)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            .addComponent(jLabel10)
                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                .addComponent(jLabel9)
                                .addComponent(btnCalcularSomaPeso))
                            .addComponent(numDeVertices1, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.PREFERRED_SIZE, 79, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addComponent(respCalcSomPes, javax.swing.GroupLayout.PREFERRED_SIZE, 79, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addComponent(textBox, javax.swing.GroupLayout.PREFERRED_SIZE, 79, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(pesquisaNoGrafo)
                    .addComponent(mostraAdjacentes)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jLabel11)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(entrarVertice, javax.swing.GroupLayout.PREFERRED_SIZE, 79, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addContainerGap(53, Short.MAX_VALUE))
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addContainerGap()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            .addComponent(jLabel8)
                            .addComponent(txtQtdeVertices, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(btnCriarVertices)
                            .addComponent(jButton1))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            .addComponent(jLabel2)
                            .addComponent(cmbOrigem, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(jLabel3)
                            .addComponent(cmbDestino, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(jLabel5)
                            .addComponent(txtPeso, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(btnInserirAresta))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(jLabel4)
                            .addComponent(jScrollPane1, javax.swing.GroupLayout.PREFERRED_SIZE, 197, javax.swing.GroupLayout.PREFERRED_SIZE)))
                    .addGroup(layout.createSequentialGroup()
                        .addGap(26, 26, 26)
                        .addComponent(jLabel9)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(numDeVertices1, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addGap(12, 12, 12)
                        .addComponent(btnCalcularSomaPeso)
                        .addGap(54, 54, 54)
                        .addComponent(jLabel10)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(respCalcSomPes, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jLabel7)
                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                        .addComponent(txtVerticeInicial, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addComponent(txtMenorCaminho)))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(jLabel6)
                            .addComponent(jScrollPane2, javax.swing.GroupLayout.PREFERRED_SIZE, 208, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addContainerGap(21, Short.MAX_VALUE))
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(textBox, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addGap(18, 18, 18)
                        .addComponent(pesquisaNoGrafo)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            .addComponent(jLabel11)
                            .addComponent(entrarVertice, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addGap(18, 18, 18)
                        .addComponent(mostraAdjacentes)
                        .addGap(30, 30, 30))))
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void btnInserirArestaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInserirArestaActionPerformed
        g.addAresta(Integer.parseInt(this.cmbOrigem.getSelectedItem().toString()),
                Integer.parseInt(this.cmbDestino.getSelectedItem().toString()),
                Integer.parseInt(this.txtPeso.getText()));

        this.txtarGrafo.setText(g.toString());
    }//GEN-LAST:event_btnInserirArestaActionPerformed

    private void btnCriarVerticesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCriarVerticesActionPerformed
        int QtdeVertices;
        QtdeVertices = Integer.parseInt(this.txtQtdeVertices.getText());
        /*aqui cria o grafo e adiciona a quantidade de vertices solicitada*/
        g = new Grafo(QtdeVertices);

        /*agora tenho que popular os combosbox para que seja possível
        construir as arestas*/
        for (int i = 0; i < QtdeVertices; i++) {
            this.cmbOrigem.addItem(i);
            this.cmbDestino.addItem(i);
        }
    }//GEN-LAST:event_btnCriarVerticesActionPerformed

    private void txtMenorCaminhoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtMenorCaminhoActionPerformed
        // Calculate Dijkstra.
        g.calcular(g.getVertice(Integer.parseInt(this.txtVerticeInicial.getText())));

        this.txtarDijkstra.setText(g.gerarStringDijkstra());
    }//GEN-LAST:event_txtMenorCaminhoActionPerformed

    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        g = new Grafo(9);

        // Add the required edges.
        g.addAresta(0, 1, 4);
        g.addAresta(0, 7, 8);
        g.addAresta(1, 2, 8);
        g.addAresta(1, 7, 11);
        g.addAresta(2, 1, 8);
        g.addAresta(2, 8, 2);
        g.addAresta(2, 5, 4);
        g.addAresta(2, 3, 7);
        g.addAresta(3, 2, 7);
        g.addAresta(3, 5, 14);
        g.addAresta(3, 4, 9);
        g.addAresta(4, 3, 9);
        g.addAresta(4, 5, 10);
        g.addAresta(5, 4, 10);
        g.addAresta(5, 3, 9);
        g.addAresta(5, 2, 4);
        g.addAresta(5, 6, 2);
        g.addAresta(6, 7, 1);
        g.addAresta(6, 8, 6);
        g.addAresta(6, 5, 2);
        g.addAresta(7, 0, 8);
        g.addAresta(7, 8, 7);
        g.addAresta(7, 1, 11);
        g.addAresta(7, 6, 1);
        g.addAresta(8, 2, 2);
        g.addAresta(8, 7, 7);
        g.addAresta(8, 6, 6);

        this.txtarGrafo.setText(g.toString());
    }//GEN-LAST:event_jButton1ActionPerformed

    private void btnCalcularSomaPesoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCalcularSomaPesoActionPerformed
        int vertEscolhido = Integer.parseInt(this.numDeVertices1.getText());

        if (vertEscolhido >= g.getVertices().size()) {
            JOptionPane.showMessageDialog(null, "Vertice nao encontrado");
        } else {
            for (int i = 0; i < g.getVertices().size(); i++) {
                if (g.getVertice(i).nome.equals("v" + Integer.toString(vertEscolhido))) {
                    this.respCalcSomPes.setText("" + g.somaPeso(vertEscolhido));
                }
            }
        }
    }//GEN-LAST:event_btnCalcularSomaPesoActionPerformed

    private void numDeVertices1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_numDeVertices1ActionPerformed
    }//GEN-LAST:event_numDeVertices1ActionPerformed

    private void respCalcSomPesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_respCalcSomPesActionPerformed
    }//GEN-LAST:event_respCalcSomPesActionPerformed

    private void textBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_textBoxActionPerformed
    }//GEN-LAST:event_textBoxActionPerformed

    private void pesquisaNoGrafoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_pesquisaNoGrafoActionPerformed
        int qtdEscolhida = Integer.parseInt(this.textBox.getText());
        
        JOptionPane.showMessageDialog(null, "\nQuantidade de Vertices que possuem a mesma incidencia de: " + qtdEscolhida + "\nEh de: " + g.varreGrafo(qtdEscolhida));
    }//GEN-LAST:event_pesquisaNoGrafoActionPerformed

    private void entrarVerticeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_entrarVerticeActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_entrarVerticeActionPerformed

    private void mostraAdjacentesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_mostraAdjacentesActionPerformed
        int qtdEscolhida = Integer.parseInt(this.entrarVertice.getText());
        
        g.mostraAdjacentes(qtdEscolhida);
        
    }//GEN-LAST:event_mostraAdjacentesActionPerformed

    /**
     * @param args the command line arguments
     */
    public static void main(String args[]) {
        /* Set the Nimbus look and feel */
        //<editor-fold defaultstate="collapsed" desc=" Look and feel setting code (optional) ">
        /* If Nimbus (introduced in Java SE 6) is not available, stay with the default look and feel.
         * For details see http://download.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html 
         */
        try {
            for (javax.swing.UIManager.LookAndFeelInfo info : javax.swing.UIManager.getInstalledLookAndFeels()) {
                if ("Nimbus".equals(info.getName())) {
                    javax.swing.UIManager.setLookAndFeel(info.getClassName());
                    break;
                }
            }
        } catch (ClassNotFoundException ex) {
            java.util.logging.Logger.getLogger(Janela1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (InstantiationException ex) {
            java.util.logging.Logger.getLogger(Janela1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (IllegalAccessException ex) {
            java.util.logging.Logger.getLogger(Janela1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (javax.swing.UnsupportedLookAndFeelException ex) {
            java.util.logging.Logger.getLogger(Janela1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>

        /* Create and display the form */
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Janela1().setVisible(true);
            }
        });
    }

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton btnCalcularSomaPeso;
    private javax.swing.JButton btnCriarVertices;
    private javax.swing.JButton btnInserirAresta;
    private javax.swing.JComboBox cmbDestino;
    private javax.swing.JComboBox cmbOrigem;
    private javax.swing.JTextField entrarVertice;
    private javax.swing.JButton jButton1;
    private javax.swing.JLabel jLabel10;
    private javax.swing.JLabel jLabel11;
    private javax.swing.JLabel jLabel2;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JLabel jLabel5;
    private javax.swing.JLabel jLabel6;
    private javax.swing.JLabel jLabel7;
    private javax.swing.JLabel jLabel8;
    private javax.swing.JLabel jLabel9;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JScrollPane jScrollPane2;
    private javax.swing.JButton mostraAdjacentes;
    private javax.swing.JTextField numDeVertices1;
    private javax.swing.JButton pesquisaNoGrafo;
    private javax.swing.JTextField respCalcSomPes;
    private javax.swing.JTextField textBox;
    private javax.swing.JButton txtMenorCaminho;
    private javax.swing.JTextField txtPeso;
    private javax.swing.JTextField txtQtdeVertices;
    private javax.swing.JTextField txtVerticeInicial;
    private javax.swing.JTextArea txtarDijkstra;
    private javax.swing.JTextArea txtarGrafo;
    // End of variables declaration//GEN-END:variables
}
